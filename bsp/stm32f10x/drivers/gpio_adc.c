...
struct gpio_adc_user_data adc_ntc1_user_data = 
{
    DEVICE_NAME_ADC_NTC1,
    /* gpio */
    GPIOB,
    GPIO_Pin_0,
    GPIO_Mode_AIN,
...
gpio_device adc_ntc1_device;

int rt_hw_adc_ntc1_register(void)
{
    gpio_device *adc_device = &adc_ntc1_device;
    struct gpio_adc_user_data *adc_user_data = &adc_ntc1_user_data;
...
struct gpio_adc_user_data adc_ntc2_user_data = 
{
    DEVICE_NAME_ADC_NTC2,
    /* gpio */
    GPIOB,
    GPIO_Pin_0,
    GPIO_Mode_AIN,
...
gpio_device adc_ntc2_device;

int rt_hw_adc_ntc2_register(void)
{
    gpio_device *adc_device = &adc_ntc2_device;
    struct gpio_adc_user_data *adc_user_data = &adc_ntc2_user_data;
    adc_user_data->dma_memory_base_address = (uint32_t)&(adc_user_data->adc_converted_value);
    adc_device->ops = &gpio_adc_user_ops;
    rt_hw_gpio_register(adc_device,adc_user_data->name, (RT_DEVICE_FLAG_RDONLY | RT_DEVICE_FLAG_DMA_RX), adc_user_data);
    return 0;
...
struct gpio_adc_user_data adc_battery_user_data = 
{
    DEVICE_NAME_ADC_BATTERY,
    /* gpio */
    GPIOA,
    GPIO_Pin_4,
    GPIO_Mode_AIN,
...
gpio_device adc_battery_device;

int rt_hw_adc_battery_register(void)
{
    gpio_device *adc_device = &adc_battery_device;
    struct gpio_adc_user_data *adc_user_data = &adc_battery_user_data;
    adc_user_data->dma_memory_base_address = (uint32_t)&(adc_user_data->adc_converted_value);
    adc_device->ops = &gpio_adc_user_ops;
    rt_hw_gpio_register(adc_device,adc_user_data->name, (RT_DEVICE_FLAG_RDONLY | RT_DEVICE_FLAG_DMA_RX), adc_user_data);
    return 0;
...
INIT_DEVICE_EXPORT(rt_hw_adc_ntc1_register);
INIT_DEVICE_EXPORT(rt_hw_adc_ntc2_register);
INIT_DEVICE_EXPORT(rt_hw_adc_battery_register);
